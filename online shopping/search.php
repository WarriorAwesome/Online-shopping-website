  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
     ...
     <form class="form-inline my-2 my-lg-0" method="post" action="search.php">
      <input class="form-control mr-sm-2" type="search" name="searchbar" paceholder="Search" aria-label="Search">
...
$str = e1($_POST['searchbar']);

if (empty($str)){
      echo '<div class="error">';
        echo "please enter in what is to be searched";
      echo '</div>';
    }
else{
          $result = mysqli_query($db,"SELECT * FROM men where ProductType LIKE'%$str%'");
          $result2 = mysqli_query($db,"SELECT * FROM woman where ProductType LIKE'%$str%'");
          if ((mysqli_num_rows($result) >= 1) || (mysqli_num_rows($result2) >= 1)) {
          if (mysqli_num_rows($result) >= 1) {
          echo "<h1  class = 'f'>Men $str</h1><hr>";

          echo "<table cellspacing = 10 cellpadding = 10 style='font-family:Futura Lt BT; font-weight:400; border-collapse: separate; border-spacing: 50px;'> <tr>";
          ...
          echo "<td class = 'imgbg'>
          <a href ='DisplayProduct.php?p=$row[1]&p2=$row[0]&p3=$row[2]'><img src ='".$row['Imagepath']."' height = 307 width = 230> </a><br> <font style = 'font-family:Rupee Foradian'> </font>&#8377 ".$row['Price']."<br>".$row['Description'];
          ...
          if (mysqli_num_rows($result2) >= 1) {
            echo "<h1  class = 'f'>Women $str</h1><hr>";
